...
app.get('/lastEndpoint', async (req, res) => {
  try {
    ...
    if (lastLog) {
      res.json({ lastEndpoint: lastLog.action, timestamp: lastLog.timestamp, method:lastLog.method, url:lastLog.url, class: lastLog._class });
    } else {
...
app.get('/mostFrequentEndpoint', async (req, res) => {
  try {
    const pogostKlic = await Log.aggregate([
      {
        $group: {
            _id: {action:'$action', url: '$url', method: '$method', timestamp:'$timestamp' },
          count: { $sum: 1 }
        }
      },
      {
